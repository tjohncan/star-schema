...
Three of Ptolemy's entries are not single stars, and they reach the sky
two different ways.
Baily 955 is ω Centauri, which has a catalogue entry of its own:
its measured position comes from Harris, cited below.
Baily 191, the nebulous tip of Perseus's right hand, and
Baily 449, the Manger in the Crab's chest, have no such entry —
so `museum/almagest/almagest_placements.csv` names the Bright
...
The judgement is ours and is CC0 — which stars mark an object,
or which object an entry is;
the coordinates remain Yale's and Harris's, cited below.
...
Baily 233 is unidentified by anyone, to our knowledge.
`atlas_placement_pinned` holds that set at one.
...
### Harris (1996, 2010 edition), Milky Way globular clusters

- **What**: The standard compilation of parameters for the Milky Way's 157
  globular clusters. Our source for the position of ω Centauri — the one
  non-stellar object Ptolemy catalogued as an ordinary star, and the same
  source Verbunt & van Gent took its position from.
- **Where**: https://physics.mcmaster.ca/~harris/mwgc.dat
- **Retrieved**: 2026-08-20
- **Terms**: Stated outright in the catalogue's header:
...
```
  ...
  and, beside it: "Please acknowledge the use of this catalog in any published
  work you derive from it." We refer to the original source at the URL above,
  charge nothing for anything, and cite in the form the author asks for.
- **We carry**: `data/prepared/globular_cluster.csv` — one row, NGC 5139
  (ω Centauri): identifier, name, J2000 position, integrated V magnitude.
  The other 156 clusters are not carried. Generated by
  `scripts/prep_sources.py` from the fetched `mwgc.dat`.
- **Cite**: Harris, W. E. (1996), *A Catalog of Parameters for Globular
  Clusters in the Milky Way*, Astronomical Journal 112, 1487 — cited as
  "Harris 1996 (2010 edition)", which is the form the catalogue requests.
- **Status**: carried (one row).
